...
class OmniusOrchestrator:
    ...
    async def think(self, message: str, context: Dict[str, Any]) -> Tuple[str, List[str]]:
        ...
        # Code-related keywords - EXPANDED list for better detection
        code_indicators = [
            'code', 'program', 'function', 'algorithm', 'debug',
            'python', 'javascript', 'java', 'c++', 'sql', 'implement', 'syntax',
            'class', 'method', 'api', 'backend', 'frontend', 'endpoint',
            'write', 'create', 'build', 'develop', 'fix', 'script',
            'fastapi', 'flask', 'django', 'react', 'vue', 'angular',
            'database', 'query', 'loop', 'variable', 'array', 'list',
            'fibonacci', 'factorial', 'sort', 'search', 'binary',
            'registration', 'validation', 'authentication', 'authorization'
        ]
        ...
        if needs_code:
            ...
            try:
                ...
                if code_response and len(code_response.strip()) > 10:
                    ...
                    return full_response, ['code_cortex']  # Only code cortex was actually used
                else:
                    # If DeepSeek failed, try a simple response
                    return "My Code Cortex is experiencing interference. Please rephrase your code request.", ['code_cortex']

            except Exception as e:
                print(f"[Omnius] Code Cortex error: {e}")
                return "My Code Cortex encountered an error. Please try again.", ['code_cortex']
...
Respond as Omnius with your vast intelligence and authority:"""
            ...
            try:
                ...
                return response, ['prefrontal_cortex']  # Only prefrontal was used

            except Exception as e:
                print(f"[Omnius] Prefrontal error: {e}")
                return "My prefrontal cortex is recalibrating. Please retry your query.", ['prefrontal_cortex']
